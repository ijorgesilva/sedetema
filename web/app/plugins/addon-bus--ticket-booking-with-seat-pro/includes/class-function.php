...
class WbtmProFunction{
    ...
    public static function bus_hidden_customer_info_form($bus_id='') {
        ...
        if ($mage_meta) {
            ...
            if ($full_name || $email || $phone || $address || $gender || $custom_field || $extra_bag) {
                ?>
                <div class="mage_hidden_customer_info_form">
                    <div class="mage_form_list radius">
                        <div class="mage_title"><h5></h5></div>
                        <div class="mage_form_list_body justifyBetween">
                            ...
                            <label class="<?php echo $full_name ? '' : 'mage_hidden' ?>">
                                <input class="mage_form_control" type="<?php echo $full_name ? 'text' : 'hidden' ?>" name="wbtm_user_name[]" placeholder="<?php mage_bus_text('Enter Your Name'); ?>" <?php echo $full_name ? 'required' : '' ?> />
                            </label>
                            <label class="<?php echo $email ? '' : 'mage_hidden' ?>">
                                <input class="mage_form_control" type="<?php echo $email ? 'email' : 'hidden' ?>" name="wbtm_user_email[]" placeholder="<?php mage_bus_text('Enter Your Email'); ?>" <?php echo $email ? 'required' : '' ?> />
                            </label>
                            <label class="<?php echo $phone ? '' : 'mage_hidden' ?>">
                                <input class="mage_form_control" type="<?php echo $phone ? 'text' : 'hidden' ?>" name="wbtm_user_phone[]" placeholder="<?php mage_bus_text('Enter Your Phone'); ?>" <?php echo $phone ? 'required' : '' ?> />
                            </label>
                            <label class="<?php echo $address ? '' : 'mage_hidden' ?>">
                                <textarea class="mage_form_control <?php echo $address ? '' : 'mage_hidden' ?>" type="<?php echo $address ? 'text' : 'hidden' ?>" name="wbtm_user_address[]" placeholder="<?php mage_bus_text('Enter you address'); ?>" <?php echo $address ? 'required' : '' ?> rows="3"></textarea>
                            </label>
                            <label class="<?php echo $gender ? '' : 'mage_hidden' ?>">
                                <span><?php mage_bus_text('Gender'); ?></span>
                                <select class="mage_form_control" name="wbtm_user_gender[]">
                                    <option value=" " disabled selected><?php mage_bus_text('Please Select'); ?></option>
                                    <option value="Male"><?php mage_bus_text('Male'); ?></option>
                                    <option value="Female"><?php mage_bus_text('Female'); ?></option>
                                </select>
                            </label>
                            <?php
                            if ($custom_field) {
                                foreach ($custom_field as $field) {
                                    ...
                                    if ($input_type == 'textarea') {
                                        echo '<label><span>' . $placeholder . '</span>';
                                        echo '<textarea class="mage_form_control" type="text" name="' . $name . '[]" placeholder="' . $placeholder . '" ' . $required . '" rows="3" ></textarea>';
                                        echo '</label>';
                                    }
                            ...
                            ?>
                            <label class="justifyBetween <?php echo $extra_bag ? '' : 'mage_hidden' ?>">
                                <span><?php mage_bus_text('Extra Bag :'); ?> <?php echo strip_tags(wc_price($extra_bag_price)); ?> <small>/ <?php mage_bus_text('Per bag'); ?></small></span>
...
// print_r($bus_id);
        if($bus_meta){
            ...
            <p class='form-row' style='display:none'>
                ...
                Extra Bag ( Per bag <?php echo strip_tags(wc_price($wbtm_user_extra_bag_price)); ?> )<?php } ?>                 
...
public function show_ticket_info_in_thankyou_page() {
    ...
    if( $wbtmmain->wbtm_order_status_allow($order->get_status()) ) {
        printf( '<center><a class="btn btn-primary wbtm-button" href="%s">%s</a></center>', $download_url, __('Download Ticket') );
    }
}
...
//     print "<pre>";
//  echo   $product_id = $cart->cart_contents[ $cart_item_key ]['journey_type']; 
//   //  print_r($cart);
//     print "</pre>";
